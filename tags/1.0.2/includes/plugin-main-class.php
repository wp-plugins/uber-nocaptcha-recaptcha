...
class ncr_base_class {
     ...
     * @since    1.0.0
     * @access   protected
     * @var      string    $data_theme   The theme of the reCaptcha API that's going to be used when generating the mark-up for the reCaptcha HTML tag..
     */
     ...
     */
        public function __construct() {
            ...
            $this->recaptcha_private_key 	=   isset($this->plugin_settings['private_key_text'])    ? $this->plugin_settings['private_key_text']    : '';

            // reCaptcha site key
            $this->recaptcha_public_key 	=   isset($this->plugin_settings['public_key_text'])     ? $this->plugin_settings['public_key_text']     : '';

            // reCaptcha theme (light/dark); light theme is set by default
            $this->data_theme               =   isset($this->plugin_settings['captcha_theme_radio']) ? $this->plugin_settings['captcha_theme_radio'] : '';

            // reCaptcha data type (audio/image); image is set by default.
            $this->data_type                =   isset($this->plugin_settings['captcha_type_radio'])   ? $this->plugin_settings['captcha_type_radio']    : '';

            // reCaptcha language
            $this->captcha_language         =   isset($this->plugin_settings['captcha_language_select']) ? $this->plugin_settings['captcha_language_select'] : '';
        ...
        public function uncr_header_script() {
                ...
                if( !wp_script_is( 'recaptcha', 'register' ) ) { // if a script with the same handle hasn't been already registered, register ours
                    ...
                    if( !empty($this->captcha_language) ) {
                        ...
                        wp_register_script( 'recaptchaGenerate', plugins_url('js/recaptcha.js', dirname(__FILE__) ), array('recaptchaAPI', 'jquery'), '1.0', false );
                    } else {
